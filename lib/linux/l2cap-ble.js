...
L2capBle.prototype.onStdoutData = function(data) {
  ...
  while ((newLineIndex = this._buffer.indexOf('\n')) !== -1) {
    ...
    if ((found = line.match(/^accept (.*)$/))) {
      clientAddress = found[1].toLowerCase();
...
L2capBle.prototype.send = function(data) {
  ...
  this._l2capBle.stdin.write(data.toString('hex') + '\n');
};
...
L2capBle.prototype.handleRequest = function(request) {
  ...
  switch(requestType) {
    ...
    case ATT_OP_EXEC_WRITE_REQ:
      response = this.handleWriteLongOrCommand(request);
      break;
...
L2capBle.prototype.handleWriteLongOrCommand = function(request) {
  var response = null;

  var requestType = request[0];
  var finalized = (requestType === ATT_OP_EXEC_WRITE_REQ);

  if (!finalized) {
    var valueHandle = request.readUInt16LE(1);
    var offset = (requestType === ATT_OP_PREP_WRITE_REQ) ? request.readUInt16LE(3) : 0;
      ...
      if (handleProperties && (handleProperties & 0x08)) {
        ...
        var callback = (function(requestType, valueHandle) {
          return function(result) {
            ...
            if (ATT_ECODE_SUCCESS === result) {
              callbackResponse = new Buffer(request);
              callbackResponse[0] = ATT_OP_PREP_WRITE_RESP;
            } else {
              callbackResponse = this.errorResponse(requestType, valueHandle, result);
            }
        ...
        if (handleSecure & 0x08 && (this._security !== 'medium' && this._security !== 'high')) {
          response = this.errorResponse(requestType, valueHandle, ATT_ECODE_AUTHENTICATION);
        } else {
          handle.attribute.emit('writeRequest', data, offset, finalized, callback, true);
        }
      } else {
        response = this.errorResponse(requestType, valueHandle, ATT_ECODE_WRITE_NOT_PERM);
      }
    ...
    while(handle = this._pendingHandles.pop()) {
      handle.attribute.emit('writeRequest', null, null, true, null, true);
    }
    var callbackResponse = new Buffer([ATT_OP_EXEC_WRITE_RESP]);
    debug('write long exce' + callbackResponse.toString('hex'));
